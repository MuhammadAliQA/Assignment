...
def make(filename, outfile):
    ...
    for l in lines:
        ...
        if l.startswith(b'#') and section == STR:
            add(msgctxt, msgid, msgstr, fuzzy)
            section = msgctxt = None
            fuzzy = 0
        # Record a fuzzy mark
        if l.startswith(b'#') and b'fuzzy' in l:
            fuzzy = 1
        # Skip comments
        if l.startswith(b'#'):
            continue
        # Now we are in a msgid or msgctxt section, output previous section
        if l.startswith(b'msgctxt'):
            if section == STR:
                add(msgctxt, msgid, msgstr, fuzzy)
            section = CTXT
            l = l[7:]
            msgctxt = b''
        elif l.startswith(b'msgid') and not l.startswith(b'msgid_plural'):
            if section == STR:
                if not msgid:
        ...
        elif l.startswith(b'msgid_plural'):
            if section != ID:
        ...
        if not l:
            continue
        l = ast.literal_eval(l.decode(encoding))
        if section == CTXT:
